...
public:
	...
	int push(const point & start,const point & end,const point & point_n,Astack *F = NULL);//将一个新节点节点加入栈中（open列表）
	int push(Astack *Astack_p);//将一个已有节点加入栈中(用于close列表)
	Astack * find_point(const point & point_n);//查找栈中是否存在以n为坐标的节点
	Astack *get_Fmin();//得到一个f最小的节点，并返回这个节点的地址
	int delete_point(Astack *Astack_p);//删除栈中一个节点，但是不释放。因为open中删除后会放入另外一个栈close中
